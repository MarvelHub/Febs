    ...
    <a-modal title="添加用户" :visible="userAddVisiable" :closable="false"
        ...
        <a-form :form="form">
            ...
            <a-form-item label='邮箱' v-bind="formItemLayout">
                <a-input v-model="user.email" v-decorator="['email',{rules: [ { type: 'email', message: '请输入正确的邮箱' },
                    { max: 50, message: '长度不能超过50个字符'} ]}]"/>
            </a-form-item>
            <a-form-item label="手机" v-bind="formItemLayout">
                <a-input v-model="user.mobile" v-decorator="['mobile', {
                    rules: [{ pattern: '^0?(13[0-9]|15[012356789]|17[013678]|18[0-9]|14[57])[0-9]{8}$',
                    message: '请输入正确的手机号'} ]}]"/>
            </a-form-item>
            <a-form-item label='角色' v-bind="formItemLayout">
                <a-select :allowClear="true" v-model="user.roleId"
                        style="width: 100%" v-decorator="['role',{rules: [{ required: true, message: '请选择角色' }]}]">
            ...
            <a-form-item label='部门' v-bind="formItemLayout">
                <a-tree-select :allowClear="true" :dropdownStyle="{ maxHeight: '220px', overflow: 'auto' }"
                        :treeData="deptTreeData" v-decorator="['deptId']"v-model="user.deptId">
                    <a-icon slot="suffixIcon" type="caret-down" />
                </a-tree-select>
            </a-form-item>
            <a-form-item label='状态' v-bind="formItemLayout">
                <a-radio-group v-model="user.status" v-decorator="['status',{rules: [{ required: true, message: '请选择状态'}]}]">
                    <a-radio value="0">锁定</a-radio>
                    <a-radio value="1">有效</a-radio>
                </a-radio-group>
            </a-form-item>
            <a-form-item label='性别' v-bind="formItemLayout">
                <a-radio-group v-model="user.ssex" v-decorator="['ssex',{rules: [{ required: true, message: '请选择性别' }]}]">
